...
server <- function(input, output) {
  ...
  observe({ 
    ...
    message <- paste("widgets sold in North Carolina:", 
                     shape$widgets[shape$uid == event$id])
  ...
  observe({ 
    ...
    message <- paste("widgets sold in", points$name[points$uid == event$id],
                     ":", points$widgets[points$uid == event$id]) 
    output$widgets <- renderText(message)
  })
}
